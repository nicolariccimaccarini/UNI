...
void init(struct configuration *config) {
    ...
    // lista di proposte.
    int n_algorithms = 6;
    char *algos[] = {"INSERTION", "MERGE", "HYBRIDMERGE", "QUICK", "MOTQUICK", "TAILQUICK", "HEAOPSORT", "INTROSORT"};
...
void _intro_sort(int *arr, int start, int end, int depht_limit) {
    if (end - start <= HYBRID_QS_K) {
        insertion_sort(arr, start, end);
    } else if (depht_limit == 0) {
        ...
        while (start < end) {
            ...
            if (mid-start > end-mid) {
                _intro_sort(arr, start, mid-1, depht_limit-1);
                start = mid+1;
            } else {
                _intro_sort(arr, mid+1, end, depht_limit-1);
                end = mid-1;
            }
        }
    }
