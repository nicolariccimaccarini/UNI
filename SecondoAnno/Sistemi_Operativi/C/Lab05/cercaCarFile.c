...
// dichiarazione del signal handler
void start_race (int);
void defeat (int);
...
int main(int argc, char **argv) {
    ...
    for (check_index=1; check_index < (unsigned int) (argc - 2); ++check_index) {
        if (strlen(argv[check_index]) != 1) {
            fprintf(stderr, "Ogni parametro C_i deve avere un singolo carattere\n");
            exit(2);
        }
    }

    // numero processi (dato il numero di caratteri inseriti)
    nProc = check_index - 1;
    ...
    // Chiudo il file, che a questo pounto esiste
    close(fd);
    file_name = argv[check_index];

    ++check_index;
    // Vado a controllare che NumSec sia effettivamente un numero intero
    ...
    /* installo i gestori dei segnali prima d creare figli
       (che erediteranno la maschera attuale) */
    ...
    for (i=0; i<nProc; i++) {
        ...
        if (pid == 0) {
            ...
            // Attendo segnale di sincronizzazione del padre
            pause();
   ...
   printf("VIA!!!\n\n");
   // incio il segnale di start a tutti i processi con lo stesso group id
   kill(0, SIGINT);

   // Il padre aspetta lo scadere del timeout
   sleep(timeout);

   // se programma termina senza vincitori
   if (!termination) {
