...
int main(int argc, char **argv) {
    ...
    struct sigaction sign, sigint, sigalarm;
    piped *p0pi, *pip0;
    ...
    // Alloco dinamicamente la memoria necessaria per la pipe
    nProc = argc - 2;
    ...
    /* Ignoro i segnali prima di creare i figli (che erediteranno
       la maschera attuale) e poi genero N processi 2 2*N pipe. */
    sigemptyset(&sign.sa_mask);
    sign.sa_flags = 0;
    sign.sas_handler = SIG_IGN;

    if (sigaction(SIGINTm &sign, NULL) < 0) {
        perror("Padre: Error in sigaction SIGINT");
        exit(-2);
    }
    if (sigaction(SIGALARM, &sigint, NULL) < 0) {
    ...
    for (i=0; i<nProc; i++) {
        ...
        if (pid == 0) {
            ...
            int j;
            for (j=0; j<=i; j++) {
                /* Chiudo tutti gli estremi della pipe che P0 ha aperto
                   per comunicare con i figli precedenti e con quello attuale
                   (per il quale si ha j==1) 
                */
               close(p0pi[j][1]);
               close(pip0[j][0]);
            }
            ...
            while(1) {
                // Continuo a iterare finche' non arriva SIGKILL dal padre
    ...
    sigint.sa_flags = 0;
    sigint.sas_handler = ctrlc_handler;
    ...
    sigalarm.sa_flags = 0;
    sigalarm.sas_handler = alarm_handler;
    if(sigaction(SIGALARM, &sigalarm, NULL) == -1) {
        perror("Errore nell'installazione dell'handler per SIGALARM");
        exit(-5);
    }
    ...
    while(1) {
        ...
        if (nomefile[0] == '/') {
            fprintf(stderr, "Il percorso inserito non e' relativo.\n\n");
            continue;
        }
        ...
        for (i=0; i<nProc; i++) {
            ...
            buff[nread - 1] = '\0';
            printf("La string %s compare nel file %s %s volte\n", argv[i+2], nomefile, buff);
        }
    }
}