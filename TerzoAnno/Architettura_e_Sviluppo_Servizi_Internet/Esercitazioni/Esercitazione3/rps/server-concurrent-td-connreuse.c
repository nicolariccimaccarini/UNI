...
int main(int argc, char** argv) {
    ...
    /* Installo gestore SIGCHLD usando la syscall sigaction, che e' uno
     ...
     * accept e' stata interrotta da un segnale e in tal caso rilanciarla */
    ...
    for (;;) {
        ...
        if ((pid = fork()) < 0) {
            ...
            for (;;) {
                ...
                // Creo un nipote a cui demandero' l'esecuzione di ps
                if ((pid2 = fork()) < 0) {
                    perror("fork");
                    exit(EXIT_FAILURE);
                } else if (pid2 == 0) {
